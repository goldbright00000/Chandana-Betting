...
$url = $tmp_odds_url = (count($uri_array) > 1 && !in_array('ongoing', $uri_array) || $uri_array[0] == "FINDIT1") ? $odds_url.$_GET['q'] : $odds_url."football";
	...
	<nav class="navbar navbar-default">
  		<div class="container-fluid">
    		<ul class="nav navbar-nav">
			  	<li <?php echo (in_array('football', $uri_array) && count($uri_array)==1) ? 'class="active"' : '' ?>><a href="<?php echo $base_url ?>/football">Today Matches</a></li>
			  	<li <?php echo (in_array('football-coupons', $uri_array)) ? 'class="active"' : '' ?>><a href="<?php echo $base_url ?>/football/football-coupons/major-leagues-cups">Major World Football</a></li>
				...
				<li <?php echo in_array('championship', $uri_array) ? 'class="active"' : '' ?>><a href="<?php echo $base_url ?>/football/english/championship">Championship</a></li>
			  	...
			  	<li <?php echo (in_array('ongoing', $uri_array)) ? 'class="active"' : '' ?>><a href="<?php echo $base_url ?>/football/ongoing">In Play Matches</a></li>
			  	<li <?php echo (in_array('FINDIT1', $uri_array)) ? 'class="active"' : '' ?>><a href="<?php echo $base_url ?>/football/FINDIT1/going">Find the Matches</a></li> 
			</ul>
		</div>
	</nav>
...
<!-- The Modal -->
<?php 
	if ($url=="http://www.oddschecker.com/football/FINDIT1/going" || $url=="http://www.oddschecker.com/FINDIT1"){
		?>
		<div id="myModal" class="modal_find">
		  <div class="modal-content">
		  	<?php
		  		if ($url=="http://www.oddschecker.com/football/FINDIT1/going"){
		  			?>
		  			<form action="<?php echo $base_url ?>/football/FINDIT1" method="get">
		  			<?php
		  		}
		  		else{
		  			?>
		  			<form action="<?php echo $base_url ?>/FINDIT1/" method="get">
		  			<?php
		  		}
		        ?>
		  	<br>
			    <div style="color: white">
			    	...
			    	<h2 style="margin-left: 100px;">Find the Matches</h2>
			    </div>
		    	<br>
			    <div class="modal-header form-group">
			      <h3>Match Name : </h3>
			      <br>
			      <input type="text" id="match_name" name="match_name" placeholder="Enter match name">
			      	<br>
			    </div>
			    <br>
			    <br>
			    ...
			    <div>
			    	<input type="submit" value="Serach" class="btn search_btn" style="margin-right: 160px;">
			    </div>
			</form>
		  </div>
		</div>
...
{
	if($url=="http://www.oddschecker.com/football/FINDIT1" || $url=="http://www.oddschecker.com/FINDIT1/"){
		$url_collection = get_url_collection_match($match_name, $market_type, $match_date);
	}
	else{
		$url_collection = get_url_collection($url, $odds);
	}
	...
	if(!empty($url_collection))
	{
		...
		{
			?>
			<button class="btn-refresh btn btn-info pull-right" onclick="location.reload(false);">Refresh</button>
			<?php
			$j=0;
			for($i=0;$i<count($result);$i++)
			{
				...
				?>
				<h1 class="heading"><?php echo $result[$i]['title'] ?> 
				
				<small><?php echo $result[$i]['date_time'] ?></small>
				
				<?php
						if(!empty($result[$i]['data']))
						{
							$html = str_get_html($result[$i]['data']);
							$trs = $html->find('tr');
							
							$home = $trs[5]->{'data-best-dig'};
						        $draw = $trs[6]->{'data-best-dig'};
							$away = $trs[7]->{'data-best-dig'};
							if ($away == 0){
							$profit = (1/$home) +(1/$draw);
                                                        }
                                                        else{
                                                        $profit = (1/$home) +(1/$draw) +(1/$away);
                                                        }
							$profit = round((100 - ($profit*100)),2);
							
							$win = $profit;
					?>
							<div class="col-xs-4 col-md-4" id="packery-calculator" style="<?php if($profit>0.01) echo 'background-color:green !important;'; else echo 'background-color: rgb(0, 0, 50) !important;' ?>display:inline-block;float:right;margin:5px;">
								<div class="item w4 h4 nc_main_element" style="height:30px;">
									<div class="new_calc standalone_nc" style="<?php if($profit>0.01) echo 'background-color:green !important;'; else echo 'background-color: background-color: rgb(111, 206, 236); !important;' ?>padding:0px;">
										<div class="new_calc_league_bet_infos">
											<div>
												<div class="new_calc_league_bet_risk_text ncl_btext valuediv">
													<input class="risk_input" onfocus="" size="5" value="100" data-home="<?php echo $home ?>" data-draw="<?php echo $draw ?>" data-away="<?php echo $away ?>"  style="<?php if($profit>0.01) echo "color:white !important" ?>">
													<span style="font-size: 110% !important;<?php if($profit>0.01) echo "color:white !important" ?>">£</span>
													<img class="left" src="http://private.oneclickodds.com/img/icons_2/edit.png"  style="<?php if($profit>0.01) echo 'background-color:green !important;'; ?>">
												</div>
												<div class="new_calc_league_bet_profit_text ncl_btext valuediv" style="color: rgb(0, 0, 0);">
													<span class="middle" style="font-size: 110% !important;<?php if($profit>0.01) echo "color:white !important" ?>"><?php echo $profit ?></span> 
													<span style="font-size: 110% !important;<?php if($profit>0.01) echo "color:white !important" ?>">%</span>
												</div>
												<div class="new_calc_league_bet_win_text ncl_btext valuediv" style="color: rgb(0, 0, 0);">
													<input class="win_input" size="5" value="<?php echo $win; ?>"  data-home="<?php echo $home ?>" data-draw="<?php echo $draw ?>" data-away="<?php echo $away ?>"  style="<?php if($profit>0.01) echo "color:white !important" ?>">
													<span style="font-size: 110% !important;<?php if($profit>0.01) echo "color:white !important" ?>">£</span>
													<img class="right" src="http://private.oneclickodds.com/img/icons_2/edit.png"  style="<?php if($profit>0.01) echo 'background-color:green !important;'; ?>">
												</div>
											</div>
										</div>
									</div>
...
$(document).ready(function(e) {
	...
	$("#packery-calculator input.risk_input").keyup(function(){
		var home = parseFloat($(this).attr('data-home'));
		var draw = parseFloat($(this).attr('data-draw'));
		var away = parseFloat($(this).attr('data-away'));
		
		var profit = 100 - parseFloat(( (1/home) + (1/draw) + (1/away))*100);
		profit = roundup(profit,2);
		$(this).parent().parent().find("div.new_calc_league_bet_profit_text span:first").html(profit.toString());
		
		var risk = parseFloat($(this).val());
		var win = profit*risk/100;
		win = roundup(win,2);
		
		$(this).parent().parent().find("input.win_input").val(win.toString());
	});

	$("#packery-calculator input.win_input").keyup(function(){
		var home = parseFloat($(this).attr('data-home'));
		var draw = parseFloat($(this).attr('data-draw'));
		var away = parseFloat($(this).attr('data-away'));
		...
		var win = parseFloat($(this).parent().parent().find("input.win_input").val())*100;
		
		var risk = roundup(win/profit,0);
		$(this).parent().parent().find("input.risk_input").val(risk.toString());
	});

	$("#packery-calculator img.left").click(function(){		
			var input = $(this).parent().parent().find("input.risk_input");
			input.focus().val(input.val());
	});
	
	$("#packery-calculator img.right").click(function(){	
		var input = $(this).parent().parent().find("input.win_input");
		input.focus().val(input.val());
	});
