...
function select_sport() {
	if (times){
		...
		if(select[0].value == "Football"){
		...
		else if (select[0].value == "Basketball") {
			$('#market_type').empty();
			var text = '<option>Winner</option><option>Point Spread</option><option>Total Points</option><option>Total Points Odd/Even</option>';
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}
		else if (select[0].value == "Rugby") {
			$('#market_type').empty();
			var text = '<option>Winner</option><option>Handicaps</option><option>First Tryscorer</option><option>Half Time/Full Time</option>';
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}
		else if (select[0].value == "Cricket") {
			$('#market_type').empty();
			var text = '<option>Winner</option><option>Top Hampshire Batsman</option><option>Top Middlesex Batsman</option><option>Highest Opening Partnership</option>';
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}
		else if (select[0].value == "Ice Hockey") {
			$('#market_type').empty();
			var text = '<option>Winner</option><option>Stanley Cup 2018 - Winning Conference</option><option>Stanley Cup 2018 - Winning Division</option><option>Stanley Cup 2018 - Winning Nation</option>';
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}
			...
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}else if (select[0].value == "American Football") {
			$('#market_type').empty();
			var text = '<option>Winner</option><option>Point Spread</option><option>Total Points</option><option>Half Time/Full Time</option>';
			$('#market_type').append(text);
			$('#search_form').attr('action', '<?php echo $base_url ?>/search_result');
		}
		else{
